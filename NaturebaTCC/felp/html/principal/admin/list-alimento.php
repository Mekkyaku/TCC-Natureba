<?php
    ...
    if ((!isset($_SESSION["logado"])) || ($_SESSION["logado"] == FALSE)){
        ...
        if ($user["tipo"] != "A"){
            ...
            if (isset($array[$min])){
                if ($x == 1){
                    $sql = "SELECT * FROM `alimentos` WHERE `ID` >= '$array[$min]' LIMIT 40;";
                }
                else if ($x == 2){
                    $sql = "SELECT * FROM `alimentos` WHERE `tag` LIKE '%{$pesquisa}%' AND `ID` >= '$array[$min]' LIMIT 40;";
                }
                else if ($x == 3){
                    $sql = "SELECT * FROM `alimentos` WHERE `categoria` = '$categoria' AND `ID` >= '$array[$min]' LIMIT 40;";
                }
                else if ($x == 4){
                    $sql = "SELECT * FROM `alimentos` WHERE `tag` LIKE '%{$pesquisa}%' AND `categoria` = '$categoria' AND `ID` >= '$array[$min]' LIMIT 40;";
                }
            }
            $res = mysqli_query($con, $sql);
        }
            ...
            <div class="conteudo">
                <div class="conteudo-body">
                    <div style="display: flex; padding-bottom: 16px;">
                        <img class="header-logo" src="../../../img/misc/icon.png">
                        <h1 class="admin">Editar alimento</h1>
                    </div>
                    <div style="padding-bottom: 32px;">
                        <h2 class="admin">Clique no nome do alimento que deseja editar</h2>
                    </div>
                    <div class="pesquisa">
                        <form action="list-alimento.php" autocomplete="off" method="get">
                            ...
                            <select class="pesquisa" id="categoria" name="categoria" style="width: 35%">
                    ...
                    <div>
                        <?php
                            ...
                            while ($linha = mysqli_fetch_array($res)){
                                ...
                                echo    "<div class='alimento'>
                                            ...
                                            <div class='alimento-down'>
                                                <a href='edit-alimento.php?id=" . $linha["ID"] . "'>
                                                    <button class='alimento-edit'>" . $linha["nome"] . "</button>
                                                </a>
                                            </div>
                                        </div>";
                                if ($vrf == 3){
                            ...
                            if ($y == 1){
                                ...
                                echo    "<div style='padding-top: 40px'>
                                            ...
                                            <a href='list-alimento.php' class='alimento-no'>
                        ...
                        <?php
                            ...
                            if ($pgs > 1){
                                ...
                                while ($i <= $pgs){
                                    echo    "<a href='list-alimento.php?page=" . $i . "&psq=" . $pesquisa . "&ctg=" . $categoria . "'>
